...
class MapTRHead(DETRHead):
    ...
    def __init__(self,
        ...
        from projects.mmdet3d_plugin.MEE.model_semantic_grouping import ContextEncodrNet
        self.mee=ContextEncodrNet('context')

        self._init_layers()

    def fuse_pts(self,feat,query,elem_num):
        n=feat.shape[1]
        fixed_num=20
        query_num=1000
        b, num, d = query.size()
        ...
        for i in range(b):
            # 创建一个mask，初始化所有值为True
            mask = torch.ones(n, dtype=torch.bool)
            
            # 根据length[i]的值更新mask，从0开始每隔fixed_num个元素设置一个False
            idx_to_remove = [j*(fixed_num+1) for j in range(elem_num[i].item())]
            mask[idx_to_remove] = False
            
            # 应用mask，去除不要的元素
            updated_feat = feat[i][mask]
            
            # 计算需要的feat的实际长度
            length = fixed_num * elem_num[i]
            # import pdb;pdb.set_trace()
            # 将updated_feat的前reduced_length个元素保留下来
            updated_feat = updated_feat[:length]
            if updated_feat.size(0) > 0:  # 确保updated_feat不为空
                # 将保留下来的updated_feat与相应的query拼接在一起
                new_query[i] = torch.cat((updated_feat, query[i][:-length]), dim=0)
                # for j in range(int(num/query_num)):
                #     new_query[i][query_num*j:query_num*(j+1)] = torch.cat((updated_feat, query[i][query_num*j:query_num*(j+1)][:-length]), dim=0)
            else:
                new_query[i] = query[i]
    ...
    def fuse_ins(self,feat,query,elem_num):
        n=feat.shape[1]
        fixed_num=20
        query_num=1000
        b, num, d = query.size()
    ...
    def forward(self, sd_points,sd_mask,mlvl_feats, lidar_feat, img_metas, prev_bev=None,  only_bev=False):
        """Forward function.
        Args:
        ...
        sd_feat=self.mee(sd_points,sd_mask)
        elem_num =torch.max(sd_mask,dim=1)[0]+1
        ...
        if self.query_embed_type == 'all_pts':
            ...
            pts_query_pos, pts_query = torch.split(pts_embeds, self.embed_dims, dim=2)
            instance_query_pos, instance_query = torch.split(instance_embeds, self.embed_dims, dim=2)
            pts_query=pts_query.expand(bs, -1, -1, -1)
            instance_query = instance_query.expand(bs, -1, -1,-1).squeeze(2)
            instance_query = self.fuse_ins(sd_feat,instance_query,elem_num)
            instance_query = instance_query.unsqueeze(2)
            query=(pts_query + instance_query).flatten(1, 2).to(dtype)
            query=self.fuse_pts(sd_feat,query,elem_num)
            query_pos=(pts_query_pos + instance_query_pos).flatten(0, 1).to(dtype).expand(bs, -1, -1)

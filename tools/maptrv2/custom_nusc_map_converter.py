...
def _fill_trainval_infos(nusc,
   ...
   #  import json
   #  newsplit_token_path = '/home/zengshuang.zs/cvpr/newsplit_token.json' 
   #  with open(newsplit_token_path, 'r') as f:
   #      newsplit_token = json.load(f)
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
      ...
      #   if sample['token'] in newsplit_token['train']:
      #       train_nusc_infos.append(info)
      #   elif sample['token'] in newsplit_token['val']:
      #       val_nusc_infos.append(info)
...
def create_nuscenes_infos(root_path,
    ...
    if test:
        ...
        data = dict(infos=train_nusc_infos, metadata=metadata)
        info_path = osp.join(out_path,
                             '{}_map_infos_temporal_train_120m.pkl'.format(info_prefix))
        mmcv.dump(data, info_path)
        data['infos'] = val_nusc_infos
        info_val_path = osp.join(out_path,
                                 '{}_map_infos_temporal_val_120m.pkl'.format(info_prefix))

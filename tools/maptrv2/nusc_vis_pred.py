...
def vis_osm(points, mask,image_dir):
        b, n, d = points.shape
        scale_factor = 10
        width = int((15 - (-15)) * scale_factor)
        height = int((30 - (-30)) * scale_factor)
        points[:,:,0]=points[:,:,0]*30
        points[:,:,1]=points[:,:,1]*60      
        for i in range(b):
            image_real = np.zeros((height,width, 3), dtype=np.uint8)
            point_real = points[i]
            att_mask = mask[i]

            # 画真实点
            for j in range(n):
                if point_real[j, 0] == 0 and point_real[j, 1] == 0:  
                        continue
                if att_mask[j]==-1:  
                        break 
                if j > 0 and att_mask[j] == att_mask[j-1] and point_real[j-1, 0] != 0 and point_real[j-1, 1] != 0:  # 如果同一线段中的点则连接成线
                    cv2.line(image_real,
                            (int((point_real[j-1, 0])*scale_factor), int((point_real[j-1, 1])*scale_factor)),
                            (int((point_real[j, 0])*scale_factor), int((point_real[j, 1])*scale_factor)),
                            (255, 255, 255), 1)
                    cv2.circle(image_real, (int((point_real[j, 0])*scale_factor), int((point_real[j, 1])*scale_factor)), 2, (255, 255, 255), -1)
                else:  # 否则只画点
                    cv2.circle(image_real, (int((point_real[j, 0])*scale_factor), int((point_real[j, 1])*scale_factor)), 2, (255, 255, 255), -1)

            # 保存图像
            cv2.imwrite(os.path.join(f'{image_dir}.png'), image_real)
...
def main():
    ...
    for i, data in enumerate(data_loader):
        ...
        # sd_points=data['sd_points']
        # sd_masks=data['sd_mask']
        ...
        # osm_map_path = osp.join(sample_dir, 'osm_map.png')
        # vis_osm(sd_points,sd_masks,osm_map_path)
